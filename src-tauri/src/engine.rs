...
impl Engine {
    ...
    pub async fn join_room(
        ...
        self.join_room_inner(room_id, room_name, name, password)
            .await
    }
    ...
    async fn join_room_inner(
        ...
        let (signal_tx, signal_rx) = signaling::connect(&ws_url)
            .await
...
async fn engine_loop(
    ...
    loop {
        // Get encoded audio rx and ice_rx from inner (if still active)
        let (encoded_rx, ice_rx) = {
            let guard = engine.lock().await;
            let Some(inner) = guard.as_ref() else {
                break; // Engine shut down
            };
            (inner.capture.encoded_rx.clone(), inner.ice_rx.clone())
        };
